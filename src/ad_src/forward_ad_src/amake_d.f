...
C   variations   of useful results: rv1 rv2 rv rc rs dxv chordv
C                enc env
...
C                xyzles:in chords:in aincs:in rv1:out rv2:out rv:out
C                rc:out rs:out dxv:out chordv:out enc:out env:out
C MAKESURF
...
C                chord2 ainc ainc_g rv1 rv2 rv rc rs dxv chordv
C                slopev slopec
C   with respect to varying inputs: xyzscal xyztran addinc xyzles
...
C-------- store strip in global data arrays
              ...
              nj(isurf) = nj(isurf) + 1
C
              rle1_diff(1, nstrip) = (1.0-f1)*xyzlel_diff(1) - (xyzlel(1
     +          )-xyzler(1))*f1_diff + f1*xyzler_diff(1)
              rle1(1, nstrip) = (1.0-f1)*xyzlel(1) + f1*xyzler(1)
              rle1_diff(2, nstrip) = (1.0-f1)*xyzlel_diff(2) - (xyzlel(2
     +          )-xyzler(2))*f1_diff + f1*xyzler_diff(2)
              rle1(2, nstrip) = (1.0-f1)*xyzlel(2) + f1*xyzler(2)
              rle1_diff(3, nstrip) = (1.0-f1)*xyzlel_diff(3) - (xyzlel(3
     +          )-xyzler(3))*f1_diff + f1*xyzler_diff(3)
              rle1(3, nstrip) = (1.0-f1)*xyzlel(3) + f1*xyzler(3)
              chord1_diff(nstrip) = (1.0-f1)*chordl_diff - (chordl-
     +          chordr)*f1_diff + f1*chordr_diff
              chord1(nstrip) = (1.0-f1)*chordl + f1*chordr
C
              rle2_diff(1, nstrip) = (1.0-f2)*xyzlel_diff(1) - (xyzlel(1
     +          )-xyzler(1))*f2_diff + f2*xyzler_diff(1)
              rle2(1, nstrip) = (1.0-f2)*xyzlel(1) + f2*xyzler(1)
              rle2_diff(2, nstrip) = (1.0-f2)*xyzlel_diff(2) - (xyzlel(2
     +          )-xyzler(2))*f2_diff + f2*xyzler_diff(2)
              rle2(2, nstrip) = (1.0-f2)*xyzlel(2) + f2*xyzler(2)
              rle2_diff(3, nstrip) = (1.0-f2)*xyzlel_diff(3) - (xyzlel(3
     +          )-xyzler(3))*f2_diff + f2*xyzler_diff(3)
              rle2(3, nstrip) = (1.0-f2)*xyzlel(3) + f2*xyzler(3)
              chord2_diff(nstrip) = (1.0-f2)*chordl_diff - (chordl-
     +          chordr)*f2_diff + f2*chordr_diff
              chord2(nstrip) = (1.0-f2)*chordl + f2*chordr
C
              rle_diff(1, nstrip) = (1.0-fc)*xyzlel_diff(1) - (xyzlel(1)
     +          -xyzler(1))*fc_diff + fc*xyzler_diff(1)
              rle(1, nstrip) = (1.0-fc)*xyzlel(1) + fc*xyzler(1)
              rle_diff(2, nstrip) = (1.0-fc)*xyzlel_diff(2) - (xyzlel(2)
     +          -xyzler(2))*fc_diff + fc*xyzler_diff(2)
              rle(2, nstrip) = (1.0-fc)*xyzlel(2) + fc*xyzler(2)
              rle_diff(3, nstrip) = (1.0-fc)*xyzlel_diff(3) - (xyzlel(3)
     +          -xyzler(3))*fc_diff + fc*xyzler_diff(3)
              rle(3, nstrip) = (1.0-fc)*xyzlel(3) + fc*xyzler(3)
              chord_diff(nstrip) = (1.0-fc)*chordl_diff - (chordl-chordr
     +          )*fc_diff + fc*chordr_diff
...
C
              chordc_diff = chord_diff(nstrip)
              chordc = chord(nstrip)
C
              temp0 = chordl/chordc
              temp = fc*chordr/chordc
              clafc_diff = clafl*((1.-fc)*(chordl_diff-temp0*chordc_diff
     +          )/chordc-temp0*fc_diff) + clafr*(chordr*fc_diff+fc*
     +          chordr_diff-temp*chordc_diff)/chordc
              clafc = clafl*((1.-fc)*temp0) + clafr*temp
C
...
C
     ...
     +                       slopel_diff, dsdx)
                CALL AKIMA_D(xasec(1, isec+1, isurf), sasec(1, isec+1, 
