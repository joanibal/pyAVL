...
C***********************************************************************
C
      SUBROUTINE CDCL_D(j, cl, cld, cd, cdd, cd_cl, cd_cld)
      IMPLICIT NONE
      INCLUDE 'AVL_d.inc'
      REAL clmin
      REAL cdmin
...
C--- Negative stall drag model (slope matches lower side, quadratic drag rise)
            ...
            cdd = (temp*2*(cl-clmin)-cdx1/(clmin-cl0))*cld
            cd = cdmin + temp*((cl-clmin)*(cl-clmin)) + cdx1*(1.0-(cl-
     +        cl0)/(clmin-cl0))
            cd_cld = cdinc*2.0*clfac*cld
...
C--- Quadratic matching negative stall and minimum drag point with zero slope
          ELSE IF (cl .LT. cl0) THEN
            cdd = (cdmin-cd0)*2*(cl-cl0)*cld/(clmin-cl0)**2
            cd = cd0 + (cdmin-cd0)*(cl-cl0)**2/(clmin-cl0)**2
            cd_cld = (cdmin-cd0)*2.0*cld/(clmin-cl0)**2
