...
C   gradient     of useful results: rv1 rv2 rv rc rs dxv chordv
C                enc env
C   with respect to varying inputs: xyzscal xyztran addinc xyzles
C                chords aincs rv1 rv2 rv rc rs dxv chordv enc env
C   RW status of diff variables: xyzscal:out xyztran:out addinc:out
C                xyzles:out chords:out aincs:out rv1:in-out rv2:in-out
C                rv:in-out rc:in-out rs:in-out dxv:in-out chordv:in-out
C                enc:in-out env:in-out
C MAKESURF
...
C                chords aincs rle chord rle1 chord1 rle2 chord2
C                ainc ainc_g rv1 rv2 rv rc rs dxv chordv slopev
C                slopec
C   with respect to varying inputs: xyzscal xyztran addinc xyzles
C                chords aincs rle chord rle1 chord1 rle2 chord2
C                ainc ainc_g rv1 rv2 rv rc rs dxv chordv slopev
...
C
     ...
     +     ycp(ksmax)
      REAL xcp_diff(kcmax), ypt_diff(ksmax), ycp_diff(ksmax)
      REAL yzlen(ksmax)
...
C---------- element inherits alpha,beta flag from surface
     ...
     +                       sloper, sloper_diff, dsdx)
                CALL POPREAL8(slopel)
                CALL AKIMA_B(xasec(1, isec, isurf), sasec(1, isec, isurf
     ...
     +                       clafc_diff, xpt, xvr, xsr, xcp, xcp_diff)
              temp_diff0 = (1.-fc)*clafl*clafc_diff/chordc
              temp_diff = clafr*clafc_diff/chordc
              fc_diff = fc_diff + chordr*temp_diff - chordl*clafl*
     +          clafc_diff/chordc
              chordr_diff = chordr_diff + fc*temp_diff
              chordc_diff = chordc_diff - fc*chordr*temp_diff/chordc - 
     +          chordl*temp_diff0/chordc
              chordl_diff = chordl_diff + temp_diff0
              CALL POPREAL8(chordc)
              chord_diff(nstrip) = chord_diff(nstrip) + chordc_diff
              chsin_diff = 0.D0
              chcos_diff = 0.D0
              DO n=ndesign,1,-1
                ...
                temp_diff = ainc_g_diff(nstrip, n)/temp0
     ...
     +          chsinl)*chsin_diff
              ...
              fc_diff = fc_diff + (chordr-chordl)*chord_diff(nstrip) + (
     +          xyzler(3)-xyzlel(3))*rle_diff(3, nstrip) + (xyzler(2)-
     +          xyzlel(2))*rle_diff(2, nstrip) + (xyzler(1)-xyzlel(1))*
     +          rle_diff(1, nstrip)
              chordl_diff = chordl_diff + (1.0-fc)*chord_diff(nstrip) + 
     +          (1.0-f2)*chord2_diff(nstrip) + (1.0-f1)*chord1_diff(
     +          nstrip)
              chordr_diff = chordr_diff + fc*chord_diff(nstrip) + f2*
     +          chord2_diff(nstrip) + f1*chord1_diff(nstrip)
              chord_diff(nstrip) = 0.D0
              xyzlel_diff(3) = xyzlel_diff(3) + (1.0-fc)*rle_diff(3, 
     +          nstrip) + (1.0-f2)*rle2_diff(3, nstrip) + (1.0-f1)*
     +          rle1_diff(3, nstrip)
              xyzler_diff(3) = xyzler_diff(3) + fc*rle_diff(3, nstrip) +
     +          f2*rle2_diff(3, nstrip) + f1*rle1_diff(3, nstrip)
              rle_diff(3, nstrip) = 0.D0
              xyzlel_diff(2) = xyzlel_diff(2) + (1.0-fc)*rle_diff(2, 
     +          nstrip) + (1.0-f2)*rle2_diff(2, nstrip) + (1.0-f1)*
     +          rle1_diff(2, nstrip)
              xyzler_diff(2) = xyzler_diff(2) + fc*rle_diff(2, nstrip) +
     +          f2*rle2_diff(2, nstrip) + f1*rle1_diff(2, nstrip)
              rle_diff(2, nstrip) = 0.D0
              xyzlel_diff(1) = xyzlel_diff(1) + (1.0-fc)*rle_diff(1, 
     +          nstrip) + (1.0-f2)*rle2_diff(1, nstrip) + (1.0-f1)*
     +          rle1_diff(1, nstrip)
              xyzler_diff(1) = xyzler_diff(1) + fc*rle_diff(1, nstrip) +
     +          f2*rle2_diff(1, nstrip) + f1*rle1_diff(1, nstrip)
              rle_diff(1, nstrip) = 0.D0
              f2_diff = (chordr-chordl)*chord2_diff(nstrip) + (xyzler(3)
     +          -xyzlel(3))*rle2_diff(3, nstrip) + (xyzler(2)-xyzlel(2))
     +          *rle2_diff(2, nstrip) + (xyzler(1)-xyzlel(1))*rle2_diff(
     +          1, nstrip)
              chord2_diff(nstrip) = 0.D0
              rle2_diff(3, nstrip) = 0.D0
              rle2_diff(2, nstrip) = 0.D0
              rle2_diff(1, nstrip) = 0.D0
              f1_diff = (chordr-chordl)*chord1_diff(nstrip) + (xyzler(3)
     +          -xyzlel(3))*rle1_diff(3, nstrip) + (xyzler(2)-xyzlel(2))
     +          *rle1_diff(2, nstrip) + (xyzler(1)-xyzlel(1))*rle1_diff(
     +          1, nstrip)
              chord1_diff(nstrip) = 0.D0
              rle1_diff(3, nstrip) = 0.D0
              rle1_diff(2, nstrip) = 0.D0
              rle1_diff(1, nstrip) = 0.D0
              CALL POPINTEGER4(nstrip)
              temp_diff0 = fc_diff/(ypt(iptr)-ypt(iptl))
              ycp_diff(ivs) = ycp_diff(ivs) + temp_diff0
              ypt_diff(iptl) = ypt_diff(iptl) - temp_diff0
              temp_diff = -((ycp(ivs)-ypt(iptl))*temp_diff0/(ypt(iptr)-
     +          ypt(iptl)))
              ypt_diff(iptr) = ypt_diff(iptr) + temp_diff
              ypt_diff(iptl) = ypt_diff(iptl) - temp_diff
              temp_diff0 = f2_diff/(ypt(iptr)-ypt(iptl))
              ypt_diff(ipt2) = ypt_diff(ipt2) + temp_diff0
              ypt_diff(iptl) = ypt_diff(iptl) - temp_diff0
              temp_diff = -((ypt(ipt2)-ypt(iptl))*temp_diff0/(ypt(iptr)-
     +          ypt(iptl)))
              ypt_diff(iptr) = ypt_diff(iptr) + temp_diff
              ypt_diff(iptl) = ypt_diff(iptl) - temp_diff
              temp_diff0 = f1_diff/(ypt(iptr)-ypt(iptl))
              ypt_diff(ipt1) = ypt_diff(ipt1) + temp_diff0
              ypt_diff(iptl) = ypt_diff(iptl) - temp_diff0
              temp_diff = -((ypt(ipt1)-ypt(iptl))*temp_diff0/(ypt(iptr)-
     +          ypt(iptl)))
              ypt_diff(iptr) = ypt_diff(iptr) + temp_diff
              ypt_diff(iptl) = ypt_diff(iptl) - temp_diff
 ...
 140          CALL POPINTEGER4(isec)
     ...
     +          ipt2)-ypt(ipt1)))
              ...
              CALL POPREAL8(ypt1)
 160          CALL POPINTEGER4(ipt2)
              CALL POPINTEGER4(ipt1)
 170          CALL POPINTEGER4(isec)
            ENDDO
...
C                chord2 ainc ainc_g rv1 rv2 rv rc dxv chordv slopev
C                slopec
        ...
        IF (branch .NE. 0) THEN
          ...
          DO i0=1,ad_count0
            IF (i0 .EQ. 1) THEN
              ...
              END IF
            ELSE
              tmp_diff14 = chordv_diff(iii)
              chordv_diff(iii) = 0.D0
              chordv_diff(ii) = chordv_diff(ii) + tmp_diff14
              tmp_diff13 = dxv_diff(iii)
              dxv_diff(iii) = 0.D0
              dxv_diff(ii) = dxv_diff(ii) + tmp_diff13
              tmp_diff12 = slopev_diff(iii)
              slopev_diff(iii) = 0.D0
              slopev_diff(ii) = slopev_diff(ii) + tmp_diff12
              tmp_diff11 = slopec_diff(iii)
              slopec_diff(iii) = 0.D0
              slopec_diff(ii) = slopec_diff(ii) + tmp_diff11
              tmp_diff10 = rc_diff(3, iii)
              rc_diff(3, iii) = 0.D0
              rc_diff(3, ii) = rc_diff(3, ii) + tmp_diff10
              tmp_diff9 = rc_diff(2, iii)
              rc_diff(2, iii) = 0.D0
              rc_diff(2, ii) = rc_diff(2, ii) - tmp_diff9
              tmp_diff8 = rc_diff(1, iii)
              rc_diff(1, iii) = 0.D0
              rc_diff(1, ii) = rc_diff(1, ii) + tmp_diff8
              tmp_diff7 = rv_diff(3, iii)
              rv_diff(3, iii) = 0.D0
              rv_diff(3, ii) = rv_diff(3, ii) + tmp_diff7
              tmp_diff6 = rv_diff(2, iii)
              rv_diff(2, iii) = 0.D0
              rv_diff(2, ii) = rv_diff(2, ii) - tmp_diff6
              tmp_diff5 = rv_diff(1, iii)
              rv_diff(1, iii) = 0.D0
              rv_diff(1, ii) = rv_diff(1, ii) + tmp_diff5
          ...
          CALL POPINTEGER4(ad_to)
          DO n=ad_to,1,-1
            tmp_diff4 = ainc_g_diff(jji, n)
            ainc_g_diff(jji, n) = 0.D0
            ainc_g_diff(jj, n) = ainc_g_diff(jj, n) + tmp_diff4
          ENDDO
          tmp_diff3 = ainc_diff(jji)
          ainc_diff(jji) = 0.D0
          ainc_diff(jj) = ainc_diff(jj) + tmp_diff3
