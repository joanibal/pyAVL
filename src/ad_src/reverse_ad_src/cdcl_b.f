...
C   gradient     of useful results: cd
...
C***********************************************************************
C
      SUBROUTINE CDCL_B(j, cl, cl_diff, cd, cd_diff, cd_cl)
      INCLUDE 'AVL.INC'
...
C
          IF (cl .LT. clmin) THEN
            cl_diff = (2*(cl-clmin)*cdinc*clfac**2-cdx1/(clmin-cl0))*
     +        cd_diff
          ELSE IF (cl .LT. cl0) THEN
            cl_diff = 2*(cl-cl0)*(cdmin-cd0)*cd_diff/(clmin-cl0)**2
          ELSE IF (cl .LT. clmax) THEN
            cl_diff = 2*(cl-cl0)*(cdmax-cd0)*cd_diff/(clmax-cl0)**2
          ELSE
            cl_diff = (2*(cl-clmax)*cdinc*clfac**2+cdx2/(clmax-cl0))*
     +        cd_diff
          END IF
        END IF
      END IF
