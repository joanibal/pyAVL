...
C$AD II-LOOP
     ...
     &                  + (RV2(3,J)-RV1(3,J))**2 )
            RCORE1 = MAX( VRCORE*CHORDV(J) , 2.0*VRCORE*DSYZ )
...
C     
      ...
      SUBROUTINE VVOR_old(BETM,IYSYM,YSYM,IZSYM,ZSYM,VRCORE,
     &     NV,RV1,RV2,NSURFV,CHORDV,
     &     NC,RC ,    NSURFC,LVTEST,
     &     WC_GAM,NCDIM)
C--------------------------------------------------------------------
...
C
      ...
      SUBROUTINE VORVELC(X,Y,Z,LBOUND,X1,Y1,Z1,X2,Y2,Z2,inv_BETA,
     &                   U,V,W, RCSQ)
C----------------------------------------------------------
...
C
      !   AB = AMAG*BMAG
c        T = (AMAG+BMAG)*(1.0 - ADB/AB) / (AXBSQ + ALSQ*RCSQ**2)
        T = (  (BSQ-ADB)/SQRT(BSQ+RCSQ)
     &       + (ASQ-ADB)/SQRT(ASQ+RCSQ) ) / (AXBSQ + ALSQ*RCSQ)
C
        U = AXB(1)*T
        V = AXB(2)*T
        W = AXB(3)*T
