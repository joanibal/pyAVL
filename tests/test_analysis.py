...
class TestAnalysisSweep(unittest.TestCase):
    ...
    def test_constrained_alpha_sweep(self):
        ...
        np.testing.assert_allclose(self.avl_solver.CL, np.array([ 1.229212, 1.322154, 1.414586, 1.506469, 1.597764, 1.688435, 1.778445, 1.867757, 1.956337, 2.044148, 2.131159 ]) , rtol=1e-4)
        np.testing.assert_allclose(self.avl_solver.CD, np.array([0.03059 , 0.033488, 0.036577, 0.039849, 0.043298, 0.046915,
       0.050691, 0.054619, 0.058687, 0.062887, 0.067207]) , rtol=1e-4)
    ...
    def test_constrained_cl_sweep(self):
        ...
        np.testing.assert_allclose(self.avl_solver.CD, np.array([0.016546, 0.018124, 0.019949, 0.022023, 0.024343, 0.02691, 0.029722, 0.032778, 0.036076, 0.039612, 0.043385]) , rtol=1e-4)
        np.testing.assert_allclose(self.avl_solver.CM, np.zeros_like(self.avl_solver.CM) , atol=1e-8)
